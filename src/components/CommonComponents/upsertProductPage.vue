<template>
  <div class="container">
        ...
        <b-col class="col-md-6">
          ...
          <div class="form-group">
            ...
            <b-button-group vertical>
              <b-button size="sm" variant="outline-secondary" @click="noImageButton()" :disabled="productObj.productPic===''?true:false "><b-icon
    ...
    <b-modal ref="mainPic-modal" hide-footer size="sm" title="照片編輯">
      ...
      ></my-image-cropper>
      <b-button class="mt-2" @click="generateImage" block>上傳照片</b-button>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const chosePic = ref();
    const mainPic = computed(() => {//顯示照片的地方
      return productObj.value.productPic===''?defaultPicUrl.value:productObj.value.productPic;
    });
    const thisPhoto = computed(() => {//編輯照片的model預設值
      return productObj.value.productPic===''?null:productObj.value.productPic;
    });
