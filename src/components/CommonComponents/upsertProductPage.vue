<template>
  <div class="container">
    <div style="background-color: white" class=" col-12 mt-3 pt-3 mb-3 pb-3 justify-content-md-center">
      <div class="row ">
        ...
        <b-col class="col-md-6">
          <!--                pic start-->
          <div class="form-group">
            <img class="output ml-5" :src="mainPic" style="border:#ADB2B6;border-style: solid;width:65%">
            <span style="vertical-align:top;" class="ml-1">
            <b-button-group vertical>
              <b-button size="sm" variant="outline-secondary" @click="noImageButton()"><b-icon
                  icon="trash"></b-icon></b-button>
              <b-button size="sm" variant="outline-secondary" @click="editImageButton()"><b-icon
    ...
    <b-modal ref="mainPic-modal" hide-footer>
      <my-image-cropper v-model="productObj.productPic"></my-image-cropper>
...
export default {
  ...
  setup() {
    ...
    const generateImage = function () {
      let url = productObj.value.productPic.generateDataUrl();
      if (!url) {
        alert('no image')
        return
      }
      imgUrl.value = url;
      productObj.value.productPic = imgUrl.value;
    }
    const noImageButton = function () {
      productObj.value.productPic='';
      imgUrl.value=defaultPicUrl.url;
    }
    const editImageButton= function () {
      imgUrl.value===''?defaultPicUrl.url:imgUrl.value
      this.$refs['mainPic-modal'].show();
    }
    return {
      imgUrl, generateImage, noImageButton,mainPic,editImageButton,
